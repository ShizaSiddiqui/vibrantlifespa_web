...
export default function BookingDisplayMain() {
    ...
    const fetchAppointmentData = async (retryCount = 5) => {
      ...
      try {
        const response = await fetch(
          "http://localhost:8000/createAppoinmentCart",
          {
            method: "POST",
            headers: {
  ...
  useEffect(() => {
    const fetchAvailableDates = async () => {
      ...
      try {
        const response = await fetch(
          "http://localhost:8000/appointmentAvailableDatesSlots",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
  ...
  const handleDateClick = async (selectedDate) => {
    ...
    try {
      const response = await fetch(
        "http://localhost:8000/appointmentAvailableTimeSlots",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleSubmit = async (e) => {
    ...
    let Notes = `Type:Digital- Name: ${updatedFirstName} ${updatedLastName}-Email: ${email}-Phone:${formattedPhoneNumber}`;

    try {
      if (true) {
        const createClientResponse = await fetch(
          "http://localhost:8000/addClientNoteToAppoinmentCart",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
        ...
        const checkoutResponse = await fetch(
          "http://localhost:8000/checkoutAppoinmentCart",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
  ...
  const handleTimeSelection = async (time, timeId) => {
    ...
    try {
      const response = await fetch(
        "http://localhost:8000/addSelectedTimeToCart",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleAestheticianChange = async (e) => {
    ...
    if (variantId && selectedItemId && cartId) {
      try {
        const response = await fetch(
          "http://localhost:8000/addItemtoAppoinmentCart",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
    ...
    <>
      ...
      <div className="font-sans min-h-screen w-full flex items-center justify-center bg-gray-50 py-12 px-4">
        <div className="max-w-md w-full bg-white rounded-lg shadow-lg space-y-8 p-6">
          <h1 className="text-2xl font-bold text-[#5FD4D0] mb-6 text-center">
            ...
            (isFirstVisit === true || selectedProcedure) && (
              <div ref={personalInfoRef}>
                ...
                <div className="mt-4 p-4  rounded">
                  ...
                  <p>
                    Full Name: <strong>{firstName || "Not provided"}</strong>
                  </p>
                  ...
                  <p>
                    Mobile Number: <strong>{mobile || "Not provided"}</strong>
                  </p>
                </div>
              </div>
            )}
